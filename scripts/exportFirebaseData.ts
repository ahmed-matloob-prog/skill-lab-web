 ...
 *
 * Usage:
 *   npx ts-node scripts/exportFirebaseData.ts
 *
 * Or add to package.json scripts:
 *   "export-firebase": "ts-node scripts/exportFirebaseData.ts"
 */
...
async function exportCollection(db: any, collectionName: string): Promise<any[]> {
...
async function main() {
  ...
  if (!firebaseConfig.projectId) {
    ...
    console.error('   You can load them with: source .env or use dotenv');
    process.exit(1);
  }
